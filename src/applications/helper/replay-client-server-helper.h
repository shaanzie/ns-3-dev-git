 ...
 */
class ReplayServerHelper
{
  public:
    /**
     * Create ReplayServerHelper which will make life easier for people trying
     ...
     */
    ReplayServerHelper(uint16_t port);
...
class ReplayClientHelper
{
  public:
    ...
    ReplayClientHelper(Address ip, uint16_t port);
    /**
     *  Create UdpClientHelper which will make life easier for people trying
     * to set up simulations with udp-client-server. Use this variant with
    ...
    ReplayClientHelper(Address addr);
